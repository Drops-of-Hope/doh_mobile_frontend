...
export const useRoleBasedAccess = () => {
  ...
  return {
    ...
    hasFeatureAccess: (feature: string) =>
      RoleBasedAccess.hasFeatureAccess(currentRole, feature),
    ...
    getAvailableFeatures: () => {
      const features =
        RoleBasedAccess.features[
          currentRole as keyof typeof RoleBasedAccess.features
        ];
      return features || [];
    },
    ...
    canVolunteer: () =>
      hasRole(USER_ROLES.VOLUNTEER) ||
      hasRole(USER_ROLES.CAMP_ORGANIZER) ||
      hasRole(USER_ROLES.ADMIN),

    // Check if user can manage campaigns - organizations, camp organizers, and admin
    canManageCampaigns: () =>
      hasRole(USER_ROLES.ORGANIZATION) ||
      hasRole(USER_ROLES.CAMP_ORGANIZER) ||
      hasRole(USER_ROLES.ADMIN),

    // Check if user can access support services
    canAccessSupport: () =>
      hasRole(USER_ROLES.BENEFICIARY) || hasRole(USER_ROLES.ADMIN),
  };
};
